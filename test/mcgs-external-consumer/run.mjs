...
const MCGS_ARCHIVE = `https://codeload.github.com/iteathen/CUDA-MCGS/tar.gz/${MCGS_REVISION}`;
...
async function runCase(id, body) {
  try { await body(); cases.push({ id, status: 'pass' }); console.log(`case=${id} result=pass`); }
  catch (error) { cases.push({ id, status: 'fail', error: { name: error?.name ?? null, code: error?.code ?? null, message: error?.message ?? String(error) } }); console.error(`case=${id} result=fail error=${JSON.stringify(error?.message ?? String(error))}`); }
}
...
async function gitObject(...args) { const { stdout } = await execFile(gitCommand, ['-C', repositoryRoot, 'rev-parse', ...args], { maxBuffer: 1024 * 1024 }); return stdout.trim(); }
function sourceTextSha256(bytes) { const text = new TextDecoder('utf-8', { fatal: true }).decode(bytes); return createHash('sha256').update(Buffer.from(text.replace(/\r\n?/g, '\n'), 'utf8')).digest('hex'); }
...
async function installExactMcgs(tempRoot) {
  ...
  const { stdout } = await execNpm(['pack', MCGS_ARCHIVE, '--json', '--pack-destination', packRoot], { cwd: tempRoot, maxBuffer: 32 * 1024 * 1024 });
  ...
  await execNpm(['install', '--ignore-scripts', '--no-audit', '--no-fund', '--package-lock=false', tarball], { cwd: consumerRoot, maxBuffer: 32 * 1024 * 1024 });
...
async function schemaShas(shim) {
  const result = {};
  for (const name of SCHEMAS) { const url = shim.schemaUrl(`${name}-profile.schema.json`); result[name] = sourceTextSha256(await readFile(new URL(url))); }
  return result;
}
...
try {
  ...
  await runCase('MCGS-EXT-C00-exact-installed-package-boundary', async () => {
    assert.equal(packageJson.name, 'cuda-mcgs'); assert.equal(packageJson.version, '0.0.0-dev.0'); assert.equal(packageJson.private, true);
    assert.equal(installed.packed.name, 'cuda-mcgs'); assert.equal(installed.packed.version, '0.0.0-dev.0'); assert.match(installed.packed.shasum, /^[0-9a-f]{40}$/); assert.match(installed.packed.integrity, /^sha512-/); assert.equal(MCGS_ARCHIVE.endsWith(MCGS_REVISION), true);
  ...
  await runCase('MCGS-EXT-C01-public-evaluator-free-owner-normalization', () => {
    ...
    assert.equal(Object.isFrozen(authority), true); assert.equal(composed.graph.normalized.mode, 'stateless'); assert.equal(composed.policy.normalized.evaluatorMode, 'absent'); assert.deepEqual(composed.policy.normalized.value, { kind: 'none' }); assert.deepEqual(composed.policy.normalized.backup, { kind: 'none' }); assert.equal(composed.ownerResults.some(({ normalized }) => normalized.schema === 'cuda-mcgs.evaluator-profile/0.2.0'), false); assert.equal(composed.graph.normalized.objectKinds.length, 0);
  });
  ...
  await runCase('MCGS-EXT-C02-root-resolve-compose-direct-equivalence', () => {
    ...
    assert.deepEqual(facade.publication.identity, direct.publication.identity); assert.deepEqual(facade.compositionProfile.identity, direct.compositionProfile.identity); assert.deepEqual(facade.searchProgram.identity, direct.searchProgram.identity); assert.deepEqual(facade.executionPackage.identity, direct.executionPackage.identity);
    assert.equal(facade.executionPackage.normalized.semantic.sessionProfile.kind, 'absent'); assert.equal(facade.executionPackage.normalized.semantic.stageProfile.kind, 'absent'); assert.equal(facade.executionPackage.normalized.semantic.channelProfile.kind, 'absent'); assert.equal('runtime' in library, false); assert.equal('cudaJs' in library, false);
  });

  await runCase('MCGS-EXT-F01-stale-authority-fails-and-retry-succeeds', () => {
    const authority = compiler.getAcceptedContractAuthority(); const stale = structuredClone(authority); stale.contractSet.contracts[0].sha256 = '0'.repeat(64);
    const failed = library.tryCompose(composed.resolved.normalized, stale, composed.context); assert.equal(failed.status, 'failure'); assert.equal(failed.publication, null); assert.equal(failed.diagnostic.code, 'COMPOSER_AUTHORITY_DRIFT');
    const retried = library.compose(composed.resolved.normalized, authority, composed.context); assert.equal(retried.executionPackage.normalized.schema, 'cuda-mcgs.execution-package/0.2.0');
  });

  await runCase('MCGS-EXT-F02-wrong-owner-role-context-rejected', () => {
    assert.throws(() => compiler.createProgramPackageCompositionContext(composed.resolved.normalized, { profileResults: composed.ownerResults, resourceResult: composed.progress, progressResult: composed.progress, outputResult: composed.output, sessionResult: null, stageResult: null, channelResult: null, composerContributionIdentity: composed.context.composerContributionIdentity }), (error) => error?.code === 'COMPOSER_CONTEXT_PROFILE');
  });
...
await runCase('MCGS-EXT-C03-temporary-package-state-cleaned', async () => { await assert.rejects(access(tempRoot), (error) => error?.code === 'ENOENT'); });

const failed = cases.filter(({ status }) => status !== 'pass');
const summary = {
  schema: 'vector.cuda-mcgs-external-consumer-evidence/0.1.0', status: failed.length === 0 ? 'pass' : 'failed',
  vector: { repository: 'iteathen/UCI-Arena-Vector', revision: vectorRevision, tree: vectorTree },
  cudaMcgs: { repository: 'iteathen/CUDA-MCGS', revision: MCGS_REVISION, archive: MCGS_ARCHIVE, package: 'cuda-mcgs@0.0.0-dev.0' },
  ...
  environment: { node: process.version, platform: process.platform, architecture: process.arch }, cases,
...
if (failed.length > 0) { const error = new Error(`CUDA-MCGS external-consumer conformance failed: ${failed.map(({ id }) => id).join(', ')}`); error.code = 'VECTOR_MCGS_EXTERNAL_CONSUMER'; throw error; }
